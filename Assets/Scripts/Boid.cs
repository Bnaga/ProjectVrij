...
public class Boid : MonoBehaviour
{
	...
	void Update ()
    {
        thisRigidbody.MoveRotation(Quaternion.Slerp(transform.rotation,Quaternion.LookRotation(thisRigidbody.velocity),.2f));
    ...
    {
        if (Vector3.Distance(transform.localPosition, Vector3.zero) > 3f)
        {
            //thisRigidbody.AddForce((Vector3.zero - transform.localPosition) * 0.01f);
            thisRigidbody.AddForce(Vector3.zero - transform.localPosition  * .1f* Time.deltaTime);
        }
    }

    private void warpPosition ()
    {
        if (Vector3.Distance(transform.localPosition, Vector3.zero) > 20f)
        {
            //thisRigidbody.velocity/=-2;
            thisRigidbody.velocity = Quaternion.LookRotation(Vector3.zero) * Vector3.one;
